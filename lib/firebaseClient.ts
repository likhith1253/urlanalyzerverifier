...
export async function logToRealtimeDB(payload:any){
  try{ const db = getDatabase(); await push(ref(db,'analysis_logs'), payload); }catch(e){ console.warn('realtime write failed',e); throw e; }
}

export async function hashUrl(url:string){
  if(typeof window==='undefined') return '';
  const enc = new TextEncoder(); const data = enc.encode(url);
